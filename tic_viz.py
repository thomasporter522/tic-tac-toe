...
def compute(player_taken, all_taken, victory, n):
	...
	for winner in winners:
		valid = True
		for i in winner:
			if not mem(prefix + i, player_taken): valid = False
		if valid and not mem(prefix, player_taken):
			return (prefix, False)
			
	# check if it completes a draw
	draw = True
	for i in range(1,10):
		if not mem(prefix + str(i), all_taken): 
			draw = False
	if draw and not mem(prefix, all_taken):
		return (prefix, True)
			
	return (None, False)
	
def conflicting(move, taken, n):
	return mem(move, taken)
